...
double preOutput3 = 0.0;

double preOutput4 = 0.0;

double preOutput5 = 0.0;

double preOutput6 = 0.0;
...
preOutput3 += W[i+2*N] * X[i];
preOutput4 += W[i+3*N] * X[i];
preOutput5 += W[i+4*N] * X[i];
preOutput6 += W[i+5*N] * X[i];
}
...
da[i] = loss *W[i] + loss2 *W[i+N];

W[i] = W[i] - alpha * loss *X[i]- lambda * ((W[i] > 0) - (W[i] < 0));
W[i+N] = W[i+N] - alpha * loss2 *X[i]- lambda * ((W[i+N] > 0) - (W[i+N] < 0));
